  ...
  return <section className="public-registration-form guided-registration">
    <div className="guided-workspace">
      <div className="guided-main">
        ...
        <ol className="registration-steps" aria-label="Registration progress">{["Players", "Entries & shirts", "Payment & review"].map((label, index) => <li key={label} aria-current={step === index ? "step" : undefined}><button type="button" disabled={busy || (index > step && !playersValid)} onClick={() => goToStep(index)}><span>{index + 1}</span>{label}</button></li>)}</ol>
