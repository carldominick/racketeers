...
import { PaymentProof, preparePaymentImage, uploadPaymentImage } from "./components/payment-proof";
...
function PublicRegistration({ state, onSaved }: { state: TournamentState; onSaved: () => void }) {
  ...
  const lookup = async () => {
    ...
    update(data.registration); setSavedRegistrationId(data.registration.id); setPaymentFile(null); setMode("edit");
  };
  const submit = async () => {
    ...
    try {
      ...
      if (preparedImage) {
        try {
          await uploadPaymentImage(data.registration.id, data.editPin || pin, preparedImage);
