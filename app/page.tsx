...
function PublicRegistration({ state, onSaved, organizerPin = "", initialDivisionId, initialRegistration }: { state: TournamentState; onSaved: () => void; organizerPin?: string; initialDivisionId?: string; initialRegistration?: PlayerRegistration }) {
  ...
  const lookup = async () => {
    ...
    setDraft({ ...makeDraft(), ...data.registration }); setSubmittedPin(""); setSavedRegistrationId(data.registration.id); setPaymentFile(null); setMode("edit");
  };
  const beginSecondEntry = (partner = false) => {
    ...
    setPin(submittedPin || pin); setPaymentFile(null); setMode("new"); setMessage("");
  };
  const submit = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/state", { method: "POST", headers: { "content-type": "application/json", ...(organizerPin ? { "x-organizer-pin": organizerPin } : {}) }, body: JSON.stringify({ action, pin: mode === "edit" || draft.sourceRegistrationId ? pin : undefined, registration: draft }) });
...
export default function Home() {
  ...
  return <main className={state.theme === "dark" ? "dark" : ""}>
    ...
    {accessOpen && <div className="modal-backdrop"><section className="card access-dialog" role="dialog" aria-modal="true" aria-labelledby="access-title"><p className="eyebrow">Welcome to Racketeers</p><h2 id="access-title">Choose your access</h2><p>Enter an organizer or umpire PIN, or continue without a PIN to register and watch the tournament.</p><form onSubmit={event => { event.preventDefault(); void enterAccess(); }}><label>Organizer or umpire PIN<input autoFocus type="password" inputMode="numeric" autoComplete="off" value={accessPin} onChange={e => setAccessPin(e.target.value.replace(/\D/g, "").slice(0, 10))} /></label><button className="primary" disabled={accessBusy || accessPin.length < 4}>{accessBusy ? "Checking…" : "Continue with PIN"}</button></form><p role="alert">{accessMessage}</p><button className="secondary" disabled={accessBusy} onClick={() => { setAccessOpen(false); setAccessPin(""); }}>Continue without a PIN</button></section></div>}
