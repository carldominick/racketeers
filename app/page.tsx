...
type RegistrationDraft = { id?: string; name: string; divisionId: string; partnerName: string; partnerShirtSize: string; partnerDesiredLevel: string; teamName: string; shirtSize: string; playerLevel: string; desiredLevel: string };
...
function PublicRegistration({ state, onSaved }: { state: TournamentState; onSaved: () => void }) {
  const makeDraft = (): RegistrationDraft => ({ name: "", divisionId: state.divisions[0]?.id ?? "", partnerName: "", partnerShirtSize: "M", partnerDesiredLevel: "Beginner", teamName: "", shirtSize: "M", playerLevel: "Beginner", desiredLevel: "Beginner" });
  const [mode, setMode] = useState<"landing" | "new" | "edit" | "success">("landing");
  const [pin, setPin] = useState("");
  const [draft, setDraft] = useState<RegistrationDraft>(makeDraft);
  ...
  const lookup = async () => {
    ...
    const response = await fetch("/api/state", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "lookupRegistration", pin }) });
    const data = await response.json(); setBusy(false);
    if (!response.ok || !data.registration) return setMessage(data.error || "Registration PIN did not match an entry.");
    update(data.registration); setMode("edit");
  };
  const submit = async () => {
    ...
    try {
      const response = await fetch("/api/state", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, pin: mode === "edit" ? pin : undefined, registration: draft }) });
  ...
  if (state.status !== "registration") return <section className="registration-portal"><div className="portal-hero"><p className="eyebrow">Tournament status · {state.status}</p><h2>Player registration is locked</h2><p>{state.status === "setup" ? "The organizer is still preparing the tournament. Registration will open when the tournament moves to the Registration phase." : state.status === "live" ? "The tournament is live, so new registrations and PIN-based edits are closed." : "This tournament has been completed and registration is closed."}</p></div><PhaseTracker state={state} /><section className="card locked-registration"><strong>Registration is read-only outside the Registration phase.</strong><span>Contact the organizer if a correction is required.</span></section></section>;
  if (mode === "landing") return <section className="registration-portal"><div className="portal-hero"><p className="eyebrow">Player registration</p><h2>Join the Racketeers tournament</h2><p>Register a new entry or use the private PIN provided after registration to edit an existing entry.</p></div><div className="portal-options"><article className="card"><h3>Edit my registration</h3><p>Enter your 8–10 digit registration PIN.</p><input aria-label="Registration PIN" inputMode="numeric" value={pin} onChange={(event) => setPin(event.target.value.replace(/\D/g, "").slice(0, 10))} placeholder="8–10 digit PIN" /><button type="button" className="secondary" disabled={busy || pin.length < 8 || pin.length > 10} onClick={() => void lookup()}>{busy ? "Checking…" : "Open Entry"}</button></article><article className="card accent-card"><h3>Register for the tournament</h3><p>Add both players, their individual shirt sizes, and the registered playing level.</p><button type="button" className="primary" onClick={() => { setDraft(makeDraft()); setPin(""); setSubmittedPin(""); setMessage(""); setMode("new"); }}>Start Registration</button></article></div>{message && <p className="portal-message error">{message}</p>}</section>;
  if (mode === "success") return <section className="registration-portal registration-success"><div className="portal-hero"><p className="eyebrow">Registration complete</p><h2>Your entry is saved</h2><p>The submit action is now locked so this entry cannot be accidentally registered twice.</p></div><article className="card registration-receipt"><span className="receipt-check">✓</span><h3>{message}</h3><p>Keep this private PIN. You will need it to open and edit the entry while registration is active.</p><strong className="receipt-pin">{submittedPin}</strong><div className="receipt-actions"><button type="button" className="secondary" onClick={() => { setPin(submittedPin); setMessage(""); setMode("landing"); }}>Edit Entry by PIN</button><button type="button" className="primary" onClick={() => { setDraft(makeDraft()); setPin(""); setSubmittedPin(""); setMessage(""); setMode("landing"); }}>Back to Registration Page</button></div></article></section>;
  const sizes = ["XS", "S", "M", "L", "XL", "2XL", "3XL"];
  return <section className="card public-registration-form"><div className="registration-form-heading"><div><p className="eyebrow">{mode === "new" ? "New tournament entry" : "Edit tournament entry"}</p><h2>{mode === "new" ? "Register Your Pair" : "Update Your Registration"}</h2><p>Enter each player separately so shirt sizes and playing levels stay accurate.</p></div><button type="button" className="ghost" onClick={() => { setMode("landing"); setMessage(""); }}>Back</button></div><div className="registration-entry-basics"><label>Division<select value={draft.divisionId} onChange={(event) => update({ divisionId: event.target.value, partnerName: "", teamName: "" })}>{state.divisions.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.mode}</option>)}</select></label>{division?.mode === "team" && <label>Team name<input value={draft.teamName} onChange={(event) => update({ teamName: event.target.value })} placeholder="Team name" /></label>}</div><div className="registration-player-grid"><fieldset className="person-fields"><legend><span>1</span> Player 1</legend><label className="player-name-field">Player name<input value={draft.name} onChange={(event) => update({ name: event.target.value })} placeholder="Full name" /></label><label>Shirt size<select value={draft.shirtSize} onChange={(event) => update({ shirtSize: event.target.value })}>{sizes.map((size) => <option key={size}>{size}</option>)}</select></label><label>Registered level<select value={draft.desiredLevel} onChange={(event) => update({ desiredLevel: event.target.value })}>{LEVELS.map((level) => <option key={level}>{level}</option>)}</select></label></fieldset>{division?.mode !== "singles" && <fieldset className="person-fields"><legend><span>2</span> Player 2 / Partner</legend><label className="player-name-field">Partner name<input value={draft.partnerName} onChange={(event) => update({ partnerName: event.target.value })} placeholder="Partner's full name" /></label><label>Shirt size<select value={draft.partnerShirtSize} onChange={(event) => update({ partnerShirtSize: event.target.value })}>{sizes.map((size) => <option key={size}>{size}</option>)}</select></label><label>Registered level<select value={draft.partnerDesiredLevel} onChange={(event) => update({ partnerDesiredLevel: event.target.value })}>{LEVELS.map((level) => <option key={level}>{level}</option>)}</select></label></fieldset>}</div><div className="form-submit registration-submit"><div><strong>Review before submitting</strong><span>A private 8-digit PIN will be provided for future edits.</span></div><button type="button" className="primary" disabled={busy || !draft.name.trim() || !draft.divisionId || (division?.mode === "doubles" && !draft.partnerName.trim())} onClick={() => void submit()}>{busy ? "Saving…" : mode === "new" ? "Submit Registration" : "Save Changes"}</button>{message && <p className={message.includes("saved") || message.includes("updated") ? "success" : "error"}>{message}</p>}</div></section>;
}
...
export default function Home() {
  ...
  const saveLatest = useCallback(async () => {
    ...
    try {
      ...
      if (response.status === 409) {
        ...
        const remoteMatches = new Map((data.state?.matches ?? []).map((match: Match) => [match.id, match]));
  ...
  const changeOrganizerPin = async () => {
    setPinChangeMessage("");
    if (!/^\d{4,10}$/.test(newOrganizerPin)) return setPinChangeMessage("Use 4–10 numbers for the new organizer PIN.");
  ...
  const addRegistration = (divisionId: string) => commit((current) => syncRegistrationsToEntries({ ...current, divisions: current.divisions.map((division) => division.id === divisionId ? { ...division, registrationManaged: true } : division), registrations: [...current.registrations, makeRegistration(divisionId, current.registrations.filter((item) => item.divisionId === divisionId).length)] }));
  ...
  const queueUmpireSave = (match: Match) => {
    ...
    umpireTimer.current = setTimeout(() => {
      ...
      umpireSaveQueue.current = umpireSaveQueue.current.then(async () => {
        const response = await fetch("/api/state", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ matchId: snapshot.id, pin: umpirePin, sets: snapshot.sets, status: snapshot.status }) });
        const data = await response.json();
        if (!response.ok) {
          if (response.status === 409) {
  ...
  return <main className={state.theme === "dark" ? "dark" : ""}>
    <header className="topbar"><div className="brand"><div className="mark">R</div><div><p>Racketeers</p><h1>Badminton Tournament Tracker</h1></div></div><nav className="view-switch" aria-label="View"><button className={view === "organizer" ? "active" : ""} onClick={() => setView("organizer")}>Organizer</button><button className={view === "register" ? "active" : ""} onClick={() => setView("register")}>Register</button><button className={view === "umpire" ? "active" : ""} onClick={() => setView("umpire")}>Umpire</button><button className={view === "spectator" ? "active" : ""} onClick={() => setView("spectator")}>Spectator</button><button className={view === "projector" ? "active" : ""} onClick={() => setView("projector")}>Projector</button></nav><div className="header-tools"><span className={`sync ${sync}`}>● {sync === "saved" ? "Synced" : sync === "saving" ? "Saving" : "Offline"}</span><button aria-label="Toggle theme" className="theme" onClick={() => commit({ ...state, theme: state.theme === "light" ? "dark" : "light" })}>{state.theme === "light" ? "☾" : "☀"}</button></div></header>
    {view === "organizer" && <nav className="tabs">{organizerTabs.map((item) => <button key={item.id} className={tab === item.id ? "active" : ""} onClick={() => setTab(item.id)}>{item.label}</button>)}</nav>}
    <div className={view === "projector" ? "projector-container" : "content"}>{view === "organizer" ? organizerContent : view === "register" ? <PublicRegistration state={state} onSaved={() => void fetchState(unlocked ? organizerPin : "")} /> : view === "umpire" ? umpireContent : view === "spectator" ? spectatorContent : projectorContent}</div>
