...
export function PaymentProof({ registrationId, pin, organizer = false, canUpload = false }: { registrationId: string; pin: string; organizer?: boolean; canUpload?: boolean }) {
  ...
  const removePhoto = async () => {
    if (!organizer || inFlight.current || !confirm(`Delete the payment photo for registration ${registrationId}? This removes the shared photo from every linked player and entry. This cannot be undone. The registration and payment confirmation will remain unchanged.`)) return;
    inFlight.current = true; setBusy(true); setMessage("");
    try {
