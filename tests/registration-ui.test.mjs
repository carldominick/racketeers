...
test('opening page shows an access dialog and public navigation only',()=>{
 const html=renderToStaticMarkup(React.createElement(Home));
 assert.match(html,/Choose your access/);assert.match(html,/Continue without a PIN/);
 const nav=html.match(/<nav class="view-switch"[^>]*>(.*?)<\/nav>/s)[1];
 assert.match(nav,/>Registration</);assert.match(nav,/>Spectator</);assert.doesNotMatch(nav,/>Organizer<|>Umpire<|>Projector</);
});
